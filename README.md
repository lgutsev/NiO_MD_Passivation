...
Sequential second-layer jobs deliberately use the same manual stage structure as the primary systems. Submit only the builder first with `sbatch scripts/build_sequential_systems.sbatch`. After all three prepared directories exist, submit `sbatch scripts/run_sequential_deposition_array.sbatch`. Only after all three depositions finish successfully should you submit `sbatch scripts/run_sequential_hold_array.sbatch`. The builder invokes only Python and Packmol: it does not run LAMMPS validation and does not submit deposition or hold jobs.
