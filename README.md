...
Complete builds write `deposition.in`, independent `hold-300K.in` and `hold-400K.in` branches, `equilibrate-300K.in`, and `anneal-400K.in`. They preserve the corrected hybrid LJ/Buckingham styles, geometric pseudo-LJ mixing, PPPM 1e-4 slab treatment, moving wall, and long 300/400 K continuations. Deposition keeps a lower recoil wall at the box floor active during FIRE minimization, writes `optimized.data`, `optimized.restart`, `optimized.lammpstrj`, and `optimization-summary.txt`, then heats and lowers the upper wall continuously for 600,000 steps at 0.5 fs (300 ps). The wall height is included in every thermo record. Both compressed-film holds read the same `deposited.data`, run for 1,000,000 steps at 0.5 fs (500 ps), and keep the upper wall fixed at the deposition endpoint, 69.615 Å. They write separate `held-300K.*` and `held-400K.*` results, so either temperature can be run without altering the other. The subsequent 5 ns 300 K production stage starts from `held-300K.data` and releases that compressed-wall constraint according to the automatic production-wall policy. `protocol_notes.txt` documents that the established executable decks use 500 fs pressure damping although the prose value is 1000 fs.
