...
def build(
    ...
    if primary_final:
        ...
        primary_atom_width=next(iter(primary_atom_widths))
    ...
    for t in templates:
        ...
        if primary_atom_width is not None:
            for atom in pieces["Atoms"]:
                if len(atom.fields)==7 and primary_atom_width==10:
                    atom.fields.extend(("0","0","0"))
                elif len(atom.fields)!=primary_atom_width:
                    raise ValueError("stage-2 Atoms records cannot match the stage-1 data format")
        _merge(result,pieces)
