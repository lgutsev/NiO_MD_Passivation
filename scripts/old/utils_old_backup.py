        ...
        if section == 'pair':
            ...
            if len(parts) == 4:
                try:
                    ...
                    opls_data[section][improper_type] = (k, sign, multiplicity)
                except ValueError as e:
                    print(f"Error parsing line: {line.strip()}, error: {e}")
            else:
                improper_start = False
...
def format_lj_parameters(opls_data):
    ...
    """
    lj_params = []
    for atom_type, params in opls_data['pair'].items():
        lj_params.append(f"pair_coeff {atom_type} {atom_type} lj/cut/coul/long {params['epsilon']:.3f} {params['sigma']:.3f} # {atom_type}-{atom_type}")
...
def format_bond_parameters(opls_data):
    ...
    """
    bond_params = []
    for bond_type, (k, r0) in opls_data['bond'].items():
...
def format_angle_parameters(opls_data):
    ...
    """
    angle_params = []
    for angle_type, (k, theta0) in opls_data['angle'].items():
...
def format_dihedral_parameters(opls_data):
    ...
    """
    dihedral_params = []
    for dihedral_type, (v1, v2, v3, v4) in opls_data['dihedral'].items():
...
def format_improper_parameters(opls_data):
    ...
    """
    improper_params = []
    for improper_type, params in opls_data['improper'].items():
        k, sign, multiplicity = params
...
def apply_combining_rules(opls_data, manual_lj_dict, ligand_atoms, mixing_method='LB'):
    ...
    for atom_type1 in surface_atoms:
        ...
        for atom_type2 in ligand_atoms:
            if atom_type2 not in opls_data['pair']:
                continue
            
            epsilon2 = opls_data['pair'][atom_type2]['epsilon']
            sigma2 = opls_data['pair'][atom_type2]['sigma']
